...
export const AuthProvider: React.FC<Props> = ({ children }) => {
    const [authData, setAuthData] = useState<companyData | null>(null);
    ...
    async function loadStorageData() {
        ...
        try {
            ...
            if (data && data != undefined) {
                ...
                setAuthData(user as companyData);
            }
        } catch (error: any) {
    ...
    async function signIn(user: string, password: string): Promise<boolean> {
        ...
        const userCD = await app.signInWithEmailAndPassword(user, password).catch((error) => {
            console.log(error);
            alert(error.message);
        });
        ...
        if (userCD) {
            ...
            setAuthData(userCD as companyData);
